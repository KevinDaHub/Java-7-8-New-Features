...
public class Exercise1 {
	...
	public static List<Food> filterEvenCharacterSize() {
		...
		List<Food> list = fridgeContent.stream().filter(f ->(f.getName().length()%2) ==0).collect(Collectors.toList());
	...
	public static List<Food> sortAlphabetically() {
		...
		List<Food> listSort = fridgeContent.stream().sorted(Comparator.comparing(e -> e.getName())).collect(Collectors.toList());
	...
	public static List<String> mapThreeFirstCharacters() {
		...
		return fridgeContent.stream().map(f->f.getName().substring(0,3)).collect(Collectors.toList());
	...
	public static boolean foodWithSizeExists(int size) {
		...
		boolean reflect = fridgeContent.stream().anyMatch(f->f.getName().length()==size);
	...
	public static String toSingleString() {
		...
		fridgeContent.forEach(f-> bob.append(f.getName()));
	...
	public static List<Food> getExpiredFood(int day) {
		...
		return fridgeContent.stream().filter(f->LocalDate.now().plus(Period.ofDays(day)).isAfter(f.getExpirationDate())).collect(Collectors.toList());
	}
